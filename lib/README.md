...
// Or, even
class GithubSearchStore {
    GithubSearchStore() : Super(GithubSearchState.empty());

    final query = jolt("");

    late final _result = jolt.computed.future((watch) async {
    ...
    late final state = _result.transform((stream) => stream.map((snapshot) =>
        snapshot.match(
